...
class View extends Component
{
    ...
    public function updateOrderStatusTo($status)
    {
        ...
        //     // $year  = now()->year;
        //     // $month = now()->month;
        //     // $store = Store::where('year', $year)->where('month', $month)->first();
        //     // if (!$store) {
        //     //     dd('No store data found for current year and month.');
        //     // }
        //     // dd($store);

        //     foreach ($this->orders->comissionsInfo as $key => $value) {
        //         dd([
        ...
        if (auth()->user()->abailCoin() > $this->orders->comissionsInfo->sum('take_comission')) {
            ...
            if ($this->orders->status == 'Confirm') {
                ...
                foreach ($this->orders->comissionsInfo as $value) {
                    ...
                    $store->increment('total_balance', $value->store);
                    $store->increment('current_balance', $value->store);

                    // optional: update balance
                    $balance->increment('total', $value->store);
                    $balance->increment('current', $value->store);
                }
            }
