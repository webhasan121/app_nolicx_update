...
class CoastStore extends Component {
    ...
    public function getDeta() {
        $credit = Withdraw::where(['type' => 'credit', 'status' => true])->sum('maintenance_fee');
        $debit = Withdraw::where(['type' => 'debit', 'status' => true])->sum('maintenance_fee');
        // $this->store = Withdraw::where(['type' => 'credit', 'status' => true])->sum('maintenance_fee');
        $this->store = $credit - $debit;
    }

    public function withdraw2() {
        $this->dispatch('open-modal', 'withdrawModal2');
    }
    ...
    public function submit() {
        ...
        $wt->store_req = 0;
        $wt->server_fee = 0;
        $wt->maintenance_fee = $this->amount;
        ...
        $this->dispatch('close-modal', 'withdrawModal2');
    }

    public function render() {
        // $store = Store::query()->cost()->first();
        $paymentMethod = [ 'Bkash', 'Nogod', 'Rocket', 'Bank' ];
        return view('livewire.system.store.coast-store', get_defined_vars());
    }
}
