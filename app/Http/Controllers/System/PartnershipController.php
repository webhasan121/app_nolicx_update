...
class PartnershipController extends Controller
{
    ...
    public function managementTeamReact(Request $request): Response
    {
        $find = trim((string) $request->query('find', ''));

        $applications = ManagementTeam::query()
            ->with(['user', 'responder'])
            ->when($find !== '', function ($query) use ($find) {
                $query->where(function ($builder) use ($find) {
                    $builder
        ...
        return Inertia::render('Auth/system/partnership/ManagementTeam', [
            ...
            'printUrl' => route('system.partnership.management-team.print', [
                'find' => $find,
            ]),
        ]);
    }

    public function printManagementTeamReact(Request $request): Response
    {
        $find = trim((string) $request->query('find', ''));
        ...
        return Inertia::render('Auth/system/partnership/ManagementTeamPrint', [
            'applications' => $applications->values()->map(function (ManagementTeam $app, int $index) {
                return [
                    'id' => $app->id,
                    'sl' => $index + 1,
                    'user_name' => $app->user?->name,
    ...
    public function acceptManagementTeam(int $id): RedirectResponse
    {
        ManagementTeam::where('id', $id)->update([
            'status' => 1,
            'response_by' => Auth::id(),
        ]);
    ...
    public function rejectManagementTeam(int $id): RedirectResponse
    {
        ManagementTeam::where('id', $id)->update([
            'status' => 0,
            'response_by' => Auth::id(),
    ...
    public function destroyManagementTeam(int $id): RedirectResponse
    {
        ManagementTeam::where('id', $id)->delete();
