...
class OrdersController extends Controller
{
    public function indexReact(Request $request): Response
    {
        ...
        $qf = in_array($qf, ['id', 'user_id', 'belongs_to'], true) ? $qf : 'id';
        ...
        $applyFilters = function ($target) use (&$sd, &$ed, $search, $qf, $date, $type, $status, $defaultToday) {
            if ($search) {
                $target->where($qf, $search);
            }
            ...
            switch ($date) {
                case 'today':
                    $sd = '';
                    $ed = '';
                    $target->whereDate('created_at', today());
                    break;
                case 'yesterday':
                    $target->whereDate('created_at', today()->subDay());
                    break;
                case 'between':
                    if (!empty($sd) && !empty($ed)) {
                        $target->whereBetween('created_at', [$sd, Carbon::parse($ed)->endOfDay()]);
                    } elseif (!empty($sd)) {
                        $target->whereDate('created_at', Carbon::parse($sd)->toDateString());
                    } elseif (!empty($ed)) {
                        $target->whereDate('created_at', Carbon::parse($ed)->toDateString());
                    }
                    break;
            }

            if ($date === 'all' && $defaultToday) {
                $target->whereDate('created_at', today());
            }

            if ($type) {
                $target->where('user_type', $type);
            }
        ...
        $query = Order::query()
            ...
            ->withSum('comissionsInfo as comission_sum', 'take_comission');

        $applyFilters($query);

        $orders = $query->orderBy('id', 'desc')->paginate($pagn)->withQueryString();
        $stats = $applyFilters(Order::query())
            ->selectRaw('COUNT(*) as orders')
            ->selectRaw('COALESCE(SUM(total), 0) as amount')
            ->selectRaw("SUM(CASE WHEN belongs_to_type = 'reseller' THEN 1 ELSE 0 END) as user_to_reseller")
            ->selectRaw("SUM(CASE WHEN belongs_to_type = 'vendor' THEN 1 ELSE 0 END) as reseller_to_vendor")
            ->first();
        $totalCom = 0;

        $items = $orders->getCollection()->map(function (Order $item) use (&$totalCom) {
            $comission = $this->money($item->comission_sum);
            $totalCom += $comission;
        ...
        return Inertia::render('Auth/system/orders/index', [
            ...
            'stats' => [
                ...
                'amount' => $this->money($stats->amount ?? 0),
                'user_to_reseller' => (int) ($stats->user_to_reseller ?? 0),
                'reseller_to_vendor' => (int) ($stats->reseller_to_vendor ?? 0),
            ],
            'orders' => [
                'data' => $items,
            ...
            switch ($date) {
                ...
                case 'between':
                    if ($sd && $ed) {
                        ...
                        $query->whereDate('created_at', Carbon::parse($sd)->toDateString());
                    } elseif ($ed) {
                        $query->whereDate('created_at', Carbon::parse($ed)->toDateString());
                    }
                    break;
            }
        }
