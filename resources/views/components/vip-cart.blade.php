...
<div>
    ...
    <style>
        ...
        .vip_image_overlay.show {
            display: flex;
        }

        .vip_image_overlay_inner {
            max-width: 95vw;
            max-height: 85vh;
            overflow: auto;
            padding: 12px;
    ...
    <div style="{{$style??"" }}" {{$attributes->class(['selected' => $isActive, 'unSelected' =>
        ...
        <div class="px-3 pb-3">
            ...
            <div class="vip_price_row">
                ...
                @if($item->image_url)
                <button type="button" class="vip_package_image_button" onclick="vipImageOpen{{ $item->id }}()">
    ...
    @isset($item)
    @if($item->image_url)
    <div class="vip_image_overlay" id="vipImageOverlay{{ $item->id }}" onclick="vipImageClose{{ $item->id }}()">
        <div class="vip_image_overlay_inner" onclick="event.stopPropagation()">
            <div class="mb-2 text-right">
                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="vipImageZoom{{ $item->id }}(-.25)">-</button>
                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="vipImageZoom{{ $item->id }}(.25)">+</button>
                <button type="button" class="btn btn-sm btn-outline-secondary" onclick="vipImageClose{{ $item->id }}()">Close</button>
            </div>
            <img id="vipImagePreview{{ $item->id }}" src="{{ $item->image_url }}" alt="{{ $item->name }}" style="width:420px; max-width:none">
        </div>
    </div>
    <script>
        var vipImageZoomValue{{ $item->id }} = 1;
        function vipImageOpen{{ $item->id }}() {
            vipImageZoomValue{{ $item->id }} = 1;
            document.getElementById('vipImagePreview{{ $item->id }}').style.width = '420px';
            document.getElementById('vipImageOverlay{{ $item->id }}').classList.add('show');
        }
        function vipImageClose{{ $item->id }}() {
            document.getElementById('vipImageOverlay{{ $item->id }}').classList.remove('show');
        }
        function vipImageZoom{{ $item->id }}(step) {
            vipImageZoomValue{{ $item->id }} = Math.min(3, Math.max(.5, vipImageZoomValue{{ $item->id }} + step));
            document.getElementById('vipImagePreview{{ $item->id }}').style.width = Math.round(420 * vipImageZoomValue{{ $item->id }}) + 'px';
        }
    </script>
    @endif
    @endisset
