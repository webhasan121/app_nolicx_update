<div>
  ...
  <x-dashboard.page-header>{{ __('Geolocation - Countries') }}</x-dashboard.page-header>
  ...
  <x-dashboard.container>
    <x-dashboard.section>
      ...
      <x-dashboard.section.inner>
          ...
          <table class="min-w-full divide-y divide-gray-200 text-sm" >
            <thead class="bg-gray-50" >
              <tr>
                ...
                <th class="px-4 py-3 text-left font-semibold text-gray-600" >{{ __('Name of Country') }}</th>
                <th class="px-4 py-3 text-left font-semibold text-gray-600" >{{ __('States') }}</th>
            ...
            <tbody class="divide-y divide-gray-100 bg-white" >
              @forelse($countries as $key => $country)
                <tr class="hover:bg-gray-50 transition" >
                  <td class="px-4 py-3 font-medium text-gray-700" >{{ ($countries->total() - ($countries->firstItem() + $loop->index - 1)) . '.' }}</td>
                  <td class="px-4 py-3 text-gray-700" >{{ $country->name }}</td>
                  <td class="px-4 py-3 text-gray-700" >{{ $country->states->count() }}</td>
                  <td class="px-4 py-3 text-center" >
                    <span class="px-2 py-1 rounded bg-blue-50 text-blue-700 text-xs font-semibold" >{{ $country->iso2 }}</span>
                  </td>
                  <td class="px-4 py-3 text-center" >
                    <span class="px-2 py-1 rounded bg-purple-50 text-purple-700 text-xs font-semibold" >{{ $country->iso3 }}</span>
                  </td>
                  <td class="px-4 py-3 text-center space-x-2" >
                    ...
                    <button wire:click="delete({{ $country->id }})" onclick="confirm('Delete this country?') || event.stopImmediatePropagation()"
        ...
        <div class="mt-4" >{{ $countries->links('livewire.pagination-links') }}</div>
      </x-dashboard.section.inner>
    </x-dashboard.section>
  </x-dashboard.container>

  <x-modal name="countryModal" maxWidth="md" >
    <div class="p-3" >{{ ($isEdit ? __('Update') : 'Add New') . __(' Country') }}</div>
    <hr class="my-2" >
    <div class="p-4" >
      <form wire:submit.prevent="{{ $isEdit ? 'update' : 'store' }}" class="space-y-6" >
        <div class="relative" >
          <x-input-label value="Name of Country" />
          <x-text-input type="text" wire:model.live="name" class="w-full" placeholder="Enter Country Name" />
          @error('name')
        ...
        <div class="grid grid-cols-2 gap-6" >
          ...
          <div class="relative" >
            ...
            <x-text-input type="text" wire:model.live="iso3" class="w-full" placeholder="Enter iso3 code" />
            @error('iso3')
              <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
          </div>
        ...
        <div class="flex justify-end" >
          <x-primary-button type="submit" >{{ __('Save Country') }}</x-primary-button>
        </div>
      </form>
    </div>
  </x-modal>
