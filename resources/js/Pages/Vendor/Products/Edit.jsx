...
export default function Edit() {
    ...
    const save = (e) => {
        e.preventDefault();
        form.post(route("vendor.products.update", { product: productData.encrypted_id }), {
            forceFormData: true,
        });
    };

    const moveToTrash = () => {
        router.post(route("vendor.products.trash", { product: productData.encrypted_id }));
    };

    const restoreFromTrash = () => {
        router.post(route("vendor.products.restore", { product: productData.encrypted_id }));
    };

    const eraseOldImage = (imageId) => {
        router.delete(
            route("vendor.products.images.destroy", {
                product: productData.encrypted_id,
                image: imageId,
            })
        );
    };
