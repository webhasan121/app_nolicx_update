...
export default function Countries({ countries, filters = {}, printUrl }) {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => {
            router.get(
                route("system.geolocations.countries"),
                { find: trimmedSearch },
                {
                    preserveState: true,
                    preserveScroll: true,
    ...
    const resultSummary =
        countries?.total > 0
            ? `Showing ${countries?.from ?? 0}-${countries?.to ?? 0} of ${countries?.total ?? 0} countries`
            : "No countries found";
