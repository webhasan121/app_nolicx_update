                        ...
                        <tfoot>
                            <tr className="py-2 bg-gray-200">
                                ...
                                <td className="font-bold">
                                    {formatCurrency(comissions.reduce((sum, item) => sum + Number(item.buying_price || 0), 0))}
                                </td>
                                <td className="font-bold">
                                    {formatCurrency(comissions.reduce((sum, item) => sum + Number(item.selling_price || 0), 0))}
                                </td>
                                <td className="font-bold">
                                    {formatCurrency(comissions.reduce((sum, item) => sum + Number(item.profit || 0), 0))}
                                </td>
                                <td></td>
                                <td className="font-bold">
                                    {formatCurrency(comissions.reduce((sum, item) => sum + Number(item.take_comission || 0), 0))}
                                </td>
                                <td className="font-bold">
                                    {formatCurrency(comissions.reduce((sum, item) => sum + Number(item.distribute_comission || 0), 0))}
                                </td>
                                <td className="font-bold">
                                    {formatCurrency(comissions.reduce((sum, item) => sum + Number(item.store || 0), 0))}
                                </td>
