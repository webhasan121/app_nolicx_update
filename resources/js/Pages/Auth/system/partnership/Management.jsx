...
export default function Management({ applications = { data: [] }, filters = {}, printUrl }) {
    ...
    const visit = ({ nextSearch = search, page = undefined } = {}) => {
        router.get(
            route("system.partnership.management"),
            {
    ...
    const toggleStatus = (app) => {
        ...
        if (!window.confirm(t("Are you sure you want to :action this management user?", { action }))) {
            return;
        }
    ...
    return (
        <AppLayout
            title={t("Partnership - Management Access")}
            header={<PageHeader>{t("Partnership - Management Access")}</PageHeader>}
        >
            <Head title={t("Partnership - Management Access")} />
